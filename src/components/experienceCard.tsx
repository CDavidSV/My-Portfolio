...
export default function ExperienceCard({
    ...
    return (
        <article
            role='listitem'
            className='flex flex-wrap-reverse md:flex-nowrap justify-between gap-1 md:gap-4 mb-6'
        >
            ...
            <div className='w-full md:w-fit h-fit flex justify-end'>
                {experience.ongoing && (
                    <Badge className='text-purple-400 bg-primary/45 text-nowrap'>
                        {formatDateYYYMM(experience.startDate)} - Present
                    </Badge>
                )}
                {experience.endDate ? (
                    <Badge className='text-purple-400 bg-primary/45 text-nowrap'>
                        {formatDateYYYMM(experience.startDate)} -{" "}
                        {formatDateYYYMM(experience.endDate)}
                    </Badge>
                ) : (
                    <Badge className='text-purple-400 bg-primary/45 text-nowrap'>
                        {formatDateYYYMM(experience.startDate)}
